"""MoleculeNet cold-start baseline calibration.

Runs the unedited MapLight baseline through the shared pipeline once to write per-task
baseline metrics to multi_agent_molnet/knowledge/baseline_scores.json (the normalisation
reference). Same mechanism as multi_agent_drug.calibrate_baseline, retargeted at the
MolNet provider (MAGENT_TASK=molnet) + molnet_data.

Usage:  python -m multi_agent_molnet.calibrate_baseline
"""
...
import multi_agent_molnet  # noqa: F401  (registers MolNetTaskAdapter)
...
def _run_baseline() -> dict:
    print("Running MoleculeNet baseline pipeline for calibration...")
    ...
    with tempfile.TemporaryDirectory(prefix="molnet_calib_") as tmpdir:
        tmp = Path(tmpdir)
        import shutil
        ...
        env["HARNESS_WALL_LIMIT_S"]    = "5400"    # 90 min (HIV 37k is slow)
...
def main() -> int:
    ...
    print("\nBaseline calibration complete. aggregate_score for baseline = 0.0\n"
          "Launch with: python -m multi_agent_molnet.supervisor --baseline-score 0.0")
    return 0
