...
The harness (run_trial_drug.py) will deduplicate the CSV against:
  1. TDC final test molecules
  2. Internal val molecules (scaffold-split from train_val)
...
@tool(
    ...
    description=(
        "Write external training data for a specific ADMET task (data_only mode only).\n\n"
        ...
        "  1. Identity dedup: removes molecules that match TDC test / val / train by "
        "standardized InChIKey (salt & charge variants included).\n"
        "  2. SAME-SOURCE REJECTION: if >5% of the task's TDC test molecules appear in "
        "your dataset, the WHOLE file is rejected (0 rows used). This means a dataset "
        "that is the benchmark's own/original source — e.g. Obach/Lombardo (also via "
        "re-aggregations like PKSmart), AqSolDB, the Hansen Ames set — will be "
        "REJECTED. Do not submit those, and do not resubmit a rejected source.\n"
    ...
    input_schema={
        ...
        "properties": {
            "task_name": {
                "type": "string",
                "description": "Exact TDC task name, e.g. 'herg', 'ames', 'bbb_martins'.",
            },
            "workdir": {
                "type": "string",
...
async def write_external_data(args: dict) -> dict:
    ...
    if task_name not in _valid_tasks():
        return {"ok": False, "error": f"unknown task_name {task_name!r}. "
                f"Must be one of the 22 TDC ADMET tasks."}
